   ...
   if(configLocalData != null) {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      if(apiResponse.error.toString() == 'Connection to API server failed due to internet connection') {
        ...
        if (ctx != null) {
          showCustomSnackBarWidget(apiResponse.error.toString(), ctx, snackBarType: SnackBarType.error);
        } else {
          debugPrint('Config API error (no navigator context): ${apiResponse.error}');
        }
      }
    }
    notifyListeners();
